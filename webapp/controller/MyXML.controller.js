sap.ui.define([
	"Ganeshaya/controller/BaseController",
	"sap/ui/model/json/JSONModel",
	"Ganeshaya/model/models",
    "Ganeshaya/util/LifeSaver"
], function(Controller, JSONModel, myModel, LifeSaver) {
	//'use strict';
	return Controller.extend("Ganeshaya.controller.MyXML", {
        ...
        caseConvert: LifeSaver,
        onRowSelect: function(oTableRow){
            var dataPath = oTableRow.getParameter("rowContext").getPath();
            this.getView().byId("myForm").bindElement(dataPath);
        },
        onChange: function(){
			this.oView = this.getView();

			var oModel = sap.ui.getCore().getModel("fire");

			var oModelGOT = sap.ui.getCore().getModel();            
